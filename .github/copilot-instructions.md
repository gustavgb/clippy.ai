...
Data will be stored in a workspace directory, that contains a .clippy.json file with workspace settings. The .clippy.json file has the following interface:
...
Each bookmark is stored in a markdown file next to the .clippy.json file. Each file is named by the ID of the bookmark, e.g. `1.md`. A bookmark file has the following structure:

```md
---
id: 1
title: Bookmark title from web page
tags: tag1, tag2, tag3
...
This is a user defined note that contains the user's thoughts on the bookmarked website.

Every section (including the Note) can contains multiple lines.

# AI Summary

This section will be autogenerated when the user clicks **Generate summary**.
```

The `lastUpdated` property of a bookmark is read from the file metadata for interoperability with external editors.
